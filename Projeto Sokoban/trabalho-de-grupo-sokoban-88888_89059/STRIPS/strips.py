...
class STRIPS(SearchDomain):
    ...
    def heuristic(self, state, goal):
        ...
        goals_pos=[pos.args for pos in goal] #todos os goals
        boxes_pos= [p.args for p in state if type(p).__name__ == 'Box_On' or type(p).__name__ == 'Box_On_Goal'] #todas as caixas
        unplaced_boxes = [p.args for p in state if type(p).__name__ == 'Box_On' ] #todas as caixas not on goal
        unfilled_goals = [pos for pos in goals_pos if pos not in boxes_pos] #goals que não têm caixas
        ...
        player_dist = min([distance(get_keeper_pos(state)[0], box) for box in unplaced_boxes])
        box_distances = sum([min([distance(box, goal) for goal in unfilled_goals]) for box in unplaced_boxes])
