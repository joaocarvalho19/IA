...
class Cidades(SearchDomain):
    ...
    def heuristic(self, city, goal_city):
        if self.coordinates[city] and self.coordinates[goal_city]:
            (cx, cy) = self.coordinates[city]
            (gx, gy) = self.coordinates[goal_city]

            x, y = abs(cx-gx), abs(cy - gy)
